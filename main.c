...
{
    return coap_uri_scheme_is_secure(u) ? COAPS_DEFAULT_PORT
                                        : COAP_DEFAULT_PORT;
}
...
#define BUFSIZE 100
    ...
    if(!proxy_scheme_option && proxy.host.length)
    {
        ...
        coap_insert_optlist(
            &optlist,
            coap_new_optlist(COAP_OPTION_PROXY_URI, len, (unsigned char *)arg));
    }
    else
...
static void cmdline_subscribe(char *arg)
{
    ...
    coap_insert_optlist(
        &optlist, coap_new_optlist(COAP_OPTION_OBSERVE,
...
static int cmdline_read_hint_check(const char *arg)
{
    ...
    while(fgets(tmpbuf, sizeof(tmpbuf), fp) != NULL)
    {
        ...
        if(tcp)
        {
            ...
            valid_ihs.ih_list[valid_ihs.count].hint_match =
                strndup(cp, tcp - cp);
            cp = tcp + 1;
            tcp = strchr(cp, ',');
            if(tcp)
...
static coap_dtls_pki_t *setup_pki(coap_context_t *ctx)
{
       ...
       (ca_file && strncasecmp(ca_file, "pkcs11:", 7) == 0))
    {
        ...
        dtls_pki.pki_key.key.pkcs11.private_key =
        ...
        switch(opt)
        {
            ...
            case 'L':
                ...
                if(!(block_mode & COAP_BLOCK_USE_LIBCOAP))
                {
                    fprintf(
                        stderr,
                        "Block mode must include COAP_BLOCK_USE_LIBCOAP (1)\n");
                    exit(-1);
                }
                break;
            case 'p':
            ...
            case 'o':
                ...
                output_file.s =
                    (unsigned char *)coap_malloc(output_file.length + 1);
