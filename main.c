...
#define MAX_USER                                                               \
  128              /* Maximum length of a user name (i.e., PSK                 \
                    * identity) in bytes. */
...
static void handle_sigint(int signum COAP_UNUSED) { quit = 1; }
...
static void free_xmit_data(coap_session_t *session COAP_UNUSED, void *app_ptr) {
  coap_free(app_ptr);
  return;
}
...
static coap_pdu_t *coap_new_request(coap_context_t *ctx,
  ...
  if (length) {
    ...
    coap_add_data_large_request(session, pdu, length, data, free_xmit_data,
                                data);
  }
...
static int resolve_address(const coap_str_const_t *server,
                           struct sockaddr *dst) {
...
static inline int check_token(const coap_pdu_t *received) {
...
static coap_response_t message_handler(coap_session_t *session COAP_UNUSED,
  ...
  if (!check_token(received)) {
    /* drop if this was just some message, or send RST in case of
     * notification */
    if (!sent &&
        (rcv_type == COAP_MESSAGE_CON || rcv_type == COAP_MESSAGE_NON)) {
  ...
  if (COAP_RESPONSE_CLASS(rcv_code) == 2) {
    ...
    if (!single_block_requested && block_opt) { /* handle Block2 */
      ...
      if (coap_opt_block_num(block_opt) == 0) {
        ...
        ready = doing_observe ? coap_check_option(received, COAP_OPTION_OBSERVE,
                                                  &opt_iter) == NULL
                              : 1;
      }
...
static void usage(const char *program, const char *version) {
  ...
  fprintf(stderr,
          ...
          "Copyright (C) 2010-2021 Olaf Bergmann <bergmann@tzi.org> and "
          "others\n\n"
          "%s\n"
          "%s\n",
  ...
  fprintf(
      ...
      "\tURI can be an absolute URI or a URI prefixed with scheme and "
      "host\n\n"
      "General Options\n"
      ...
      "\t-r     \t\tUse reliable protocol (TCP or TLS); requires TCP "
      "support\n"
      ...
      "\t       \t\ttext begins with 0x, then the hex text (two [0-9a-f] "
      "per\n"
      "\t       \t\tbyte) is converted to binary data\n"
      "\t-P scheme://address[:port]\tScheme, address and optional port to\n"
      "\t       \t\tdefine how to connect to a CoAP proxy (automatically "
      "adds\n"
  ...
  fprintf(
      ...
      "\tPEM format.\n\n"
      "\t-c certfile\tPEM file or PKCS11 URI for the certificate. The "
      "private\n"
      ...
      "\t       \t\tEC PRIVATE KEY. (GnuTLS and TinyDTLS(PEM) support "
      "only).\n"
...
static void cmdline_content_type(char *arg, uint16_t key) {
  ...
  node =
      coap_new_optlist(key, coap_encode_var_safe(buf, sizeof(buf), value), buf);
  if (node) {
    coap_insert_optlist(&optlist, node);
  }
}

static int cmdline_hop_limit(char *arg) {
  coap_optlist_t *node;
  uint32_t value;
  uint8_t buf[4];
  ...
  node = coap_new_optlist(COAP_OPTION_HOP_LIMIT,
                          coap_encode_var_safe(buf, sizeof(buf), value), buf);
  if (node) {
    coap_insert_optlist(&optlist, node);
  }
  return 1;
...
#define BUFSIZE 100
  ...
  if (!proxy_scheme_option && proxy.host.length) {
    ...
    coap_insert_optlist(&optlist, coap_new_optlist(COAP_OPTION_PROXY_URI, len,
                                                   (unsigned char *)arg));
    ...
    if ((uri.scheme == COAP_URI_SCHEME_COAPS_TCP ||
      ...
      coap_log(LOG_EMERG, "coaps+tcp URI scheme not supported in this "
                          "version of libcoap\n");
      return -1;
    }
    ...
    if (uri.scheme == COAP_URI_SCHEME_COAP_TCP && !coap_tcp_is_supported()) {
      ...
      coap_log(LOG_EMERG, "coap+tcp URI scheme not supported in this "
                          "version of libcoap\n");
      return -1;
    }
...
static void set_blocksize(void) {
  ...
  if (method != COAP_REQUEST_DELETE) {
    ...
    opt_length = coap_encode_var_safe(
        buf, sizeof(buf), (block.num << 4 | block.m << 3 | block.szx));
 ...
 */
static size_t convert_hex_string(const char *src, uint8_t *dst) {
  uint8_t *p = dst;
...
static void cmdline_option(char *arg) {
  ...
  if (arg[0] == '0' && arg[1] == 'x') {
    ...
    coap_insert_optlist(
        &optlist, coap_new_optlist(num, strlen(arg), (unsigned char *)arg));
  }
 ...
 */
static void decode_segment(const uint8_t *seg, size_t length,
                           unsigned char *buf) {
...
static int cmdline_input(char *text, coap_string_t *buf) {
  int len;
...
static int cmdline_input_from_file(char *filename, coap_string_t *buf) {
  FILE *inputfile = NULL;
  ssize_t len;
  int result = 1;
  struct stat statbuf;
...
static method_t cmdline_method(char *arg) {
  static const char *methods[] = {0,       "get",   "post",   "put", "delete",
                                  "fetch", "patch", "ipatch", 0};
  unsigned char i;
...
static ssize_t cmdline_read_user(char *arg, unsigned char **buf,
                                 size_t maxlen) {
...
static int verify_cn_callback(const char *cn,
                              ...
                              unsigned depth, int validated COAP_UNUSED,
...
verify_ih_callback(coap_str_const_t *hint,
                   coap_session_t *c_session COAP_UNUSED, void *arg) {
...
static coap_dtls_pki_t *setup_pki(coap_context_t *ctx) {
  ...
  if ((key_file && strncasecmp(key_file, "pkcs11:", 7) == 0) ||
    ...
    dtls_pki.pki_key.key.pkcs11.private_key = key_file ? key_file : cert_file;
    ...
    dtls_pki.pki_key.key.pem_buf.private_key_len =
        key_mem ? key_mem_len : cert_mem_len;
  }
  return &dtls_pki;
}

static coap_dtls_cpsk_t *setup_psk(const uint8_t *identity, size_t identity_len,
...
open_session(coap_context_t *ctx, coap_proto_t proto, coap_address_t *bind_addr,
  ...
  if (proto == COAP_PROTO_DTLS || proto == COAP_PROTO_TLS) {
    ...
    if (root_ca_file || ca_file || cert_file) {
      ...
      session =
          coap_new_client_session_pki(ctx, bind_addr, dst, proto, dtls_pki);
    } else if (identity || key) {
      /* Setup PSK session */
      coap_dtls_cpsk_t *dtls_psk =
          setup_psk(identity, identity_len, key, key_len);
      session =
          coap_new_client_session_psk2(ctx, bind_addr, dst, proto, dtls_psk);
    } else {
...
static coap_session_t *get_session(coap_context_t *ctx, const char *local_addr,
                                   const char *local_port, coap_proto_t proto,
                                   coap_address_t *dst, const uint8_t *identity,
                                   size_t identity_len, const uint8_t *key,
                                   size_t key_len) {
  ...
  if (local_addr) {
    ...
    hints.ai_socktype = COAP_PROTO_RELIABLE(proto)
                            ? SOCK_STREAM
                            : SOCK_DGRAM; /* Coap uses UDP */
    ...
    session = open_session(ctx, proto, NULL, dst, identity, identity_len, key,
                           key_len);
  }
  return session;
}
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv,
                       "a:b:c:e:f:h:j:k:l:m:no:p:rs:t:u:v:wA:B:C:H:J:K:L:M:NO:"
                       "P:R:T:U")) != -1) {
    switch (opt) {
    case 'a':
  ...
  session = get_session(
      ctx, node_str[0] ? node_str : NULL, port_str,
      scheme == COAP_URI_SCHEME_COAP_TCP ? COAP_PROTO_TCP
      : scheme == COAP_URI_SCHEME_COAPS_TCP
          ? COAP_PROTO_TLS
          : (reliable ? scheme == COAP_URI_SCHEME_COAPS ? COAP_PROTO_TLS
                                                        : COAP_PROTO_TCP
             : scheme == COAP_URI_SCHEME_COAPS ? COAP_PROTO_DTLS
                                               : COAP_PROTO_UDP),
      &dst, user_length >= 0 ? user : NULL, user_length >= 0 ? user_length : 0,
      key_length > 0 ? key : NULL, key_length > 0 ? key_length : 0);
  ...
  if (!uri_host_option &&
    ...
    coap_insert_optlist(
        &optlist,
        coap_new_optlist(COAP_OPTION_URI_HOST, uri.host.length, uri.host.s));
  }
  ...
  // send a second request using the same session
  if (!(pdu2 = coap_new_request(ctx, session, method, &optlist, payload.s,
                                payload.length))) {
    goto finish;
  }
  coap_log(LOG_DEBUG, "sending CoAP request 2:\n");
  if (coap_get_log_level() < LOG_DEBUG)
    coap_show_pdu(LOG_INFO, pdu2);

  coap_send(session, pdu2);

  // and continue as in the example
  ...
  while (!quit) {
    ...
    result = coap_io_process(ctx, wait_ms == 0  ? obs_ms
                                  : obs_ms == 0 ? min(wait_ms, 1000)
                                                : min(wait_ms, obs_ms));
